...
then
    ...
    wagon create --pip /$1/bin/pip --pip /$2/bin/pip --pyver "27" --pyver "36" -r ${REQUIRMENTS_FILE} -v -f -a '--no-cache-dir -c '${CONSTRAINTS_FILE}'' .
else
    echo "## $CONSTRAINTS_FILE doesn't exist"
    wagon create --pip /$1/bin/pip --pip /$2/bin/pip --pyver "27" --pyver "36" -r ${REQUIRMENTS_FILE} -v -f .
fi
